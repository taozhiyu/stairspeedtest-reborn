...
if [ ! -d libev-mingw/ ]; then # assume libev-mingw will never update again
  ...
  cd libev-mingw
  mkdir build
  ./configure --prefix="$PWD/build" --with-pcre=C:/msys2/msys64/$MSYSTEM/
  make install -j4
  cd ..
fi
...
if [ ! -d simple-obfs/ ]; then # assume simple-obfs will never update again
  ...
  ./autogen.sh
  ./configure --disable-documentation --with-ev="$LIBEV_PATH" --with-pcre=C:/msys2/msys64/$MSYSTEM/
  make -j4
else
  cd simple-obfs
fi
...
if [ ! -d shadowsocksr-libev/ ]; then # assume shadowsocksr-libev will never update again
  ...
  cd libudns
  ./autogen.sh
  ./configure --with-pcre=C:/msys2/msys64/$MSYSTEM/
  make -j4
  cd ..

  ./autogen.sh
  CFLAGS+="-fstack-protector" ./configure --disable-documentation --with-ev="$LIBEV_PATH" --with-pcre=C:/msys2/msys64/$MSYSTEM/
